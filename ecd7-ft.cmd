...
:selectadrive
FOR /f "delims=" %%a in ('mshta.exe "%tmp%\ecd7-formattingtool_%ft7sessionnumber%.tmp\GUI\Select.drive1.hta"') do set drive=%%a
if not exist "%drive%" (
start /wait mshta "javascript:alert('This drive cannot be found.');window.close()"
goto selectadrive
)
set "quickswitch="
FOR /f "delims=" %%a in ('mshta.exe "%tmp%\ecd7-formattingtool_%ft7sessionnumber%.tmp\GUI\Quickswitchmenu1.hta"') do set quick=%%a
...
%tmp%\ecd7-formattingtool_%ft7sessionnumber%.tmp\bin\7za X "%tmp%\ecd7-formattingtool_%ft7sessionnumber%.tmp\ecd7-fs_latest.7z" -o"%drive%\"
CD /D %DRIVE%
ECHO:>"\.clean"
cd /d "%tmp%\ecd7-formattingtool_%ft7sessionnumber%.tmp\"
